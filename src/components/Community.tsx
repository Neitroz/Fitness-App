  ...
  useEffect(() => {
    const fetchCommunityData = async () => {
      try {
        ...
        const prsSnap = await getDocs(prsQuery).catch(err => {
           handleFirestoreError(err, OperationType.LIST, 'manual_prs');
           throw err;
        });
        ...
        const workoutsSnap = await getDocs(workoutsQuery).catch(err => {
